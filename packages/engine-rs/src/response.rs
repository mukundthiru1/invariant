...
fn has_cmd(classes: &[InvariantClass]) -> bool {
    classes.iter().any(|c| matches!(c,
        InvariantClass::CmdSeparator | InvariantClass::CmdSubstitution | InvariantClass::CmdArgumentInjection
    ))
}
...
fn has_xss(classes: &[InvariantClass]) -> bool {
    classes.iter().any(|c| matches!(c,
        ...
        InvariantClass::XssEventHandler | InvariantClass::XssProtocolHandler | InvariantClass::XssTemplateExpression
    ))
}
...
fn has_ssrf(classes: &[InvariantClass]) -> bool {
    classes.iter().any(|c| matches!(c,
        InvariantClass::SsrfInternalReach | InvariantClass::SsrfCloudMetadata | InvariantClass::SsrfProtocolSmuggle
    ))
}

fn collect_iocs(classes: &[InvariantClass], path: &str, source_hash: &str, chains: &[ChainMatch]) -> Vec<String> {
    if classes.is_empty() && chains.is_empty() {
        return Vec::new();
    }

    let mut iocs: Vec<String> = classes.iter()
        .map(|c| format!("class:{c:?}"))
        .collect();
    iocs.push(format!("source:{source_hash}"));
    iocs.push(format!("path:{path}"));
    for chain in chains {
        iocs.push(format!("chain:{}:{:.0}%", chain.chain_id, chain.completion * 100.0));
        iocs.extend(chain.associated_sources.iter().map(|s| format!("pivot_source:{s}")));
    }
    iocs.sort_unstable();
    iocs.dedup();
    iocs
...
fn timeline_steps(severity: IncidentSeverity, chains: &[ChainMatch]) -> Vec<String> {
    ...
    if severity >= IncidentSeverity::Critical || chains.iter().any(|c| c.completion >= 0.66) {
        ...
        steps.push("Capture exact chain path + path+behavior transitions for each matched step".to_owned());
...
fn highest_severity(severities: &[&str]) -> IncidentSeverity {
    if severities.iter().any(|s| *s == "critical") { IncidentSeverity::Critical }
    else if severities.iter().any(|s| *s == "high") { IncidentSeverity::High }
    else if severities.iter().any(|s| *s == "medium") { IncidentSeverity::Medium }
...
pub fn generate_response_plan(ctx: &DetectionContext) -> ResponsePlan {
    ...
    if !ctx.classes.is_empty() {
        recs.push(IncidentRecommendation {
            ...
            urgency: if sev >= IncidentSeverity::Critical { Urgency::Immediate } else { Urgency::Within1h },
    ...
    if has_cmd(ctx.classes) {
        ...
        recs.push(IncidentRecommendation {
            ...
            urgency: Urgency::Within24h,
            category: ActionCategory::Remediate,
            action: format!("Eliminate shell command execution in {} {}", ctx.method, ctx.path),
            rationale: "Any code path that passes user input to a shell is fundamentally unsafe".into(),
            steps: vec![
    ...
    if has_xss(ctx.classes) {
        recs.push(IncidentRecommendation {
            ...
            rationale: "XSS injection detected — CSP prevents script execution even if stored".into(),
            steps: vec![
    ...
    if has_path(ctx.classes) {
        recs.push(IncidentRecommendation {
            ...
            rationale: "Path traversal may have read sensitive configuration or credential files".into(),
            steps: vec![
                format!("Check access logs for {} — look for 200 responses with large bodies", ctx.path),
    ...
    if !ctx.chains.is_empty() {
        let most_complete = ctx.chains.iter().max_by(|a, b| a.completion.partial_cmp(&b.completion).unwrap()).unwrap();
        recs.push(IncidentRecommendation {
            id: "contain_chain_escalation".into(),
            urgency: Urgency::Immediate,
            category: ActionCategory::Contain,
            action: format!("Active attack chain: {} ({}% complete)", most_complete.chain_id, (most_complete.completion * 100.0) as u32),
            rationale: "Multi-step attack in progress — attacker advancing through kill chain".into(),
            steps: vec![
    ...
    if !ctx.classes.is_empty() {
        recs.push(IncidentRecommendation {
            ...
            action: format!("Review all requests from source {} in the last 24 hours", ctx.source_hash),
    ...
    if ctx.severities.iter().any(|s| *s == "critical" || *s == "high") {
        recs.push(IncidentRecommendation {
    ...
    recs.sort_by(|a, b| {
        a.urgency.cmp(&b.urgency).then_with(|| a.category.cmp(&b.category))
    });
    ...
    let summary = if ctx.classes.is_empty() {
        ...
        format!("{} detection(s). Highest severity: {:?}.", ctx.classes.len(), sev)
    };
    ...
    ResponsePlan {
        ...
        requires_human: sev >= IncidentSeverity::Critical || ctx.chains.iter().any(|c| c.completion >= 0.66),
    }
}
...
mod tests {
    ...
    fn sql_injection_plan() {
        ...
        assert!(plan.recommendations.iter().any(|r| r.id == "contain_db_audit"));
        assert!(plan.recommendations.iter().any(|r| r.id == "remediate_sql_parameterize"));
        assert!(plan.indicators_of_compromise.iter().any(|i| i.starts_with("class:SqlUnionExtraction")));
    ...
    fn cmd_injection_plan() {
        ...
        let plan = generate_response_plan(&ctx);
        assert!(plan.recommendations.iter().any(|r| r.id == "contain_cmd_audit"));
        assert!(plan.recommendations.iter().any(|r| r.id == "remediate_cmd_eliminate"));
    ...
    fn forensic_and_timeline_recommendations() {
        ...
        let ctx = DetectionContext {
            classes: &[InvariantClass::SqlTautology, InvariantClass::SqlUnionExtraction],
        ...
        let plan = generate_response_plan(&ctx);
        assert!(plan.recommendations.iter().any(|r| r.id == "contain_temporary_automation"));
        assert!(plan.recommendations.iter().any(|r| r.id == "forensics_evidence_preservation"));
        assert!(plan.indicators_of_compromise.iter().any(|i| i.contains("chain:sqli_data_exfil")));
    ...
    fn empty_plan() {
        ...
        assert!(plan.recommendations.iter().all(|r| r.category == ActionCategory::Harden));
    }

    #[test]
    fn recommendations_ordered() {
        let ctx = DetectionContext {
            classes: &[InvariantClass::SqlTautology, InvariantClass::XssTagInjection],
            severities: &["critical", "high"],
            effect: None,
            chains: &[],
            method: "POST",
        ...
        let contain_pos = plan.recommendations.iter().position(|r| r.category == ActionCategory::Contain);
        let investigate_pos = plan.recommendations.iter().position(|r| r.category == ActionCategory::Investigate);
