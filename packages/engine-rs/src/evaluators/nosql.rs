...
const MONGO_OPERATORS: &[&str] = &[
    ...
    "$exists", "$regex", "$where", "$elemMatch", "$not",
    "$or", "$and", "$nor", "$type", "$mod", "$all",
    "$size", "$expr",
];
...
const DANGEROUS_PIPELINES: &[&str] = &[
    "$lookup", "$graphLookup", "$merge", "$out", "$unionWith", "$function",
];

const EXFIL_PIPELINES: &[&str] = &[
    "$lookup", "$merge", "$out",
];
...
impl L2Evaluator for NoSqlEvaluator {
    fn id(&self) -> &'static str { "nosql" }
    fn prefix(&self) -> &'static str { "L2 NoSQL" }
    ...
    fn detect(&self, input: &str) -> Vec<L2Detection> {
        ...
        static op_re: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r#"\$(?:gt|gte|lt|lte|ne|nin|in|exists|regex|where|elemMatch|not|or|and|nor|type|mod|all|size|expr)\b"#).unwrap());
        for m in op_re.find_iter(&decoded) {
            let op = m.as_str();
            let is_dangerous = DANGEROUS_OPERATORS.contains(&op);

            dets.push(L2Detection {
                detection_type: if is_dangerous { "nosql_code_exec" } else { "nosql_operator" }.into(),
        ...
        static JSON_OPERATOR_VALUE_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r#"(?is)\{\s*[^\{\}]{0,260}:\s*\{\s*(?:"|')?(\$[a-zA-Z][a-zA-Z0-9_]+)"#).unwrap()
        });
        ...
        for caps in json_operator_value.captures_iter(&decoded) {
            if let Some(opm) = caps.get(1) {
                let op = opm.as_str();
                if dets.iter().any(|d| d.detail.contains(op) && d.position == opm.start()) {
                    continue;
                }
                dets.push(L2Detection {
                    ...
                    evidence: vec![ProofEvidence {
                        operation: EvidenceOperation::PayloadInject,
                        matched_input: op.to_owned(),
                        interpretation: format!("Query object value starts a MongoDB operator {}", op),
                        offset: opm.start(),
        ...
        static JS_WHERE_RE: std::sync::LazyLock<Regex> =
            std::sync::LazyLock::new(|| Regex::new(r#"(?i)\$where\s*:\s*['"]?(?:function|this\.|return\s)"#).unwrap());
        ...
        if let Some(m) = js_where.find(&decoded) {
            dets.push(L2Detection {
                ...
                evidence: vec![ProofEvidence {
                    ...
                    offset: m.start(),
                    property: "User input must not inject executable code into NoSQL queries".into(),
                }],
            });
        }

        // Regex operator abuse and potential ReDoS-style patterns
        static REGEX_OPS_RE: std::sync::LazyLock<Regex> =
            std::sync::LazyLock::new(|| Regex::new(r#"(?is)\$regex\s*:\s*(?:"([^"]*)"|'([^']*)')"#).unwrap());
        let regex_ops = &*REGEX_OPS_RE;
        for caps in regex_ops.captures_iter(&decoded) {
            let pattern = caps.get(1).or_else(|| caps.get(2)).map(|g| g.as_str()).unwrap_or("");
            let confidence = if pattern.contains(".*") || pattern.contains("(.+") || pattern.contains("{") {
                0.87
            } else {
                0.82
            };
            dets.push(L2Detection {
                ...
                evidence: vec![ProofEvidence {
                    ...
                    interpretation: "User-controlled regex pattern is injected into NoSQL query".into(),
                    offset: caps.get(0).map(|m| m.start()).unwrap_or(0),
                    property: "NoSQL regex operators must reject untrusted user-controlled patterns".into(),
                }],
            });
        }

        // Timing attacks via $where + sleep()
        static where_sleep: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| Regex::new(r#"(?is)\$where\b[^\n\r}]{0,400}(sleep\(|thread\.sleep|java\.lang\.Thread\.sleep|Date\.now\(\)\s*\+\s*\d+)"#).unwrap());
        ...
        static PIPELINE_INJECTION_RE: std::sync::LazyLock<Regex> =
            std::sync::LazyLock::new(|| Regex::new(r#"(?is)\[\s*\{\s*(?:"|')?(\$[a-zA-Z][a-zA-Z0-9]*)"#).unwrap());
        ...
        static MONGO_EXFIL_PIPELINE_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
        ...
        static MONGO_WHERE_FUNCTION_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            ...
            if !full.is_empty() {
                ...
                dets.push(L2Detection {
                    ...
                    evidence: vec![ProofEvidence {
                        ...
                        interpretation: "Injected Redis Lua script may execute privileged commands".into(),
                        offset: pos,
                        property: "Redis EVAL/EVALSHA must never run user-controlled scripts".into(),
                    }],
                });
            }
        }
        ...
        static REDIS_CONFIG_SET_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r#"(?is)\bCONFIG\s+SET\b\s+(?:dir|dbfilename|appendfilename|requirepass)\b"#).unwrap()
        });
        ...
        static CASSANDRA_ALLOW_FILTERING_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r#"(?is)\bALLOW\s+FILTERING\b"#).unwrap()
        });
        if let Some(m) = CASSANDRA_ALLOW_FILTERING_RE.find(&decoded) {
            dets.push(L2Detection {
                ...
                evidence: vec![ProofEvidence {
                    operation: EvidenceOperation::PayloadInject,
                    matched_input: m.as_str().to_owned(),
                    interpretation: "ALLOW FILTERING in user-driven CQL can bypass intended query constraints".into(),
                    offset: m.start(),
        ...
        for caps in DYNAMODB_FILTER_EXPR_RE.captures_iter(&decoded) {
            let expr_key = caps.get(1).map(|m| m.as_str()).unwrap_or("FilterExpression");
            let pos = caps.get(0).map(|m| m.start()).unwrap_or(0);
            dets.push(L2Detection {
                detection_type: "nosql_operator".into(),
                confidence: 0.89,
        ...
        static DYNAMODB_ATTR_EXISTS_RE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r#"(?is)\b(attribute_exists|attribute_not_exists)\s*\("#).unwrap()
        });
        for caps in DYNAMODB_ATTR_EXISTS_RE.captures_iter(&decoded) {
            let fn_name = caps.get(1).map(|m| m.as_str()).unwrap_or("attribute_exists");
            let pos = caps.get(0).map(|m| m.start()).unwrap_or(0);
            dets.push(L2Detection {
                detection_type: "nosql_operator".into(),
                confidence: 0.87,
                ...
                evidence: vec![ProofEvidence {
                    operation: EvidenceOperation::PayloadInject,
                    matched_input: caps.get(0).map(|m| m.as_str().to_owned()).unwrap_or_default(),
                    interpretation: format!("{} can be abused when condition expressions are user-controlled", fn_name),
                    offset: pos,
                    property: "DynamoDB conditional functions must not be directly user-injected".into(),
                }],
            });
        }
        ...
        for m in NOSQL_TIME_BLIND_RE.find_iter(&decoded) {
            dets.push(L2Detection {
                detection_type: "nosql_code_exec".into(),
                confidence: 0.95,
                detail: "Time-based blind NoSQL injection pattern in JavaScript query context".into(),
                position: m.start(),
                evidence: vec![ProofEvidence {
                    operation: EvidenceOperation::SemanticEval,
                    matched_input: m.as_str().to_owned(),
                    interpretation: "Detected timing primitive consistent with blind NoSQL injection probing".into(),
                    offset: m.start(),
                    property: "NoSQL JS execution contexts must reject timing primitives from user input".into(),
                }],
            });
        }
                ...
                if key.starts_with('$') {
                    dets.push(L2Detection {
                        ...
                        evidence: vec![ProofEvidence {
                            ...
                            offset: 0,
                            property: "JSON keys starting with $ must be validated in NoSQL query context".into(),
                        }],
                    });
                }
...
mod tests {
    ...
    fn detects_json_operator_value_pattern() {
        ...
        assert!(classes.iter().any(|c| *c == InvariantClass::NosqlOperatorInjection));
    }
    ...
    fn detects_pipeline_lookup_and_timing_where() {
        ...
        let classes = detect_classes(input);
        assert!(classes.iter().any(|c| *c == InvariantClass::NosqlJsInjection));
        assert!(classes.iter().any(|c| *c == InvariantClass::NosqlOperatorInjection));
    }
    ...
    fn detects_time_based_blind_nosql_injection_patterns() {
        let input = r#"{"$where":"function(){ while(Date.now() < Date.now()+5000){}; return true; }"}"#;
        let classes = detect_classes(input);
        assert!(classes.iter().any(|c| *c == InvariantClass::NosqlJsInjection));
    }
}
