...
impl L2Evaluator for DeserEvaluator {
    ...
    fn detect(&self, input: &str) -> Vec<L2Detection> {
        ...
        let has_java_magic = decoded
            .as_bytes()
            .windows(4)
            .any(|w| w == [0xAC, 0xED, 0x00, 0x05]);
        ...
        static java_gadget: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r"(?i)(?:InvokerTransformer|ChainedTransformer|ConstantTransformer|MethodInvokeTypeProvider|AbstractBeanFactoryBasedTargetSource|com\.sun\.rowset\.JdbcRowSetImpl|javax\.management(?:\.[A-Za-z0-9_$]+)?)").unwrap()
        });
        if let Some(m) = java_gadget.find(&decoded) {
            dets.push(mk(
                "java_gadget_chain",
                0.94,
                &format!("Java gadget chain token: {}", m.as_str()),
        ...
        static java_ysoserial: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r"(?i)(?:TemplatesImpl|AnnotationInvocationHandler|BadAttributeValueExpException|PriorityQueue|BeanComparator|JdbcRowSetImpl|com\.mchange\.v2\.c3p0\.WrapperConnectionPoolDataSource)").unwrap()
        });
        ...
        if let Some(m) = php_obj.find(&decoded) {
            dets.push(mk(
                "php_object",
                0.90,
                &format!("PHP serialized object: {}", m.as_str()),
        ...
        static php_pop: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r#"(?i)(?:Monolog\\+Handler\\+[A-Za-z0-9_]+|Guzzle\\+[A-Za-z0-9_\\]+|Laravel\\+[A-Za-z0-9_\\]+|Symfony\\+[A-Za-z0-9_\\]+)"#).unwrap()
        });
        if let Some(m) = php_pop.find(&decoded) {
            dets.push(mk(
                "php_pop_chain",
                0.92,
                &format!("PHP POP chain token: {}", m.as_str()),
        ...
        if let Some(pos) = decoded.find("Tzo") {
            dets.push(mk(
                "php_base64_serial",
                0.82,
                "Potential base64-encoded PHP serialized object marker (Tzo)",
        ...
        static php_magic: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r#"(?i)(?:__wakeup|__destruct|__toString|__call|__invoke)"#).unwrap()
        });
        ...
        if let Some(m) = phar_wrapper.find(&decoded) {
            dets.push(mk(
                "php_phar_deser",
                0.90,
                "PHAR stream wrapper reference (implicit PHP deserialization path)",
        ...
        // Python pickle
        static pickle: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r"(?:c__builtin__|cos\nsystem|cposix|c__main__)").unwrap()
        });
        if let Some(m) = pickle.find(&decoded) {
            dets.push(mk(
                "python_pickle",
                0.92,
                "Python pickle with code execution opcodes",
        ...
        static py_reduce: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r"(?i)(?:__reduce_ex?__|copyreg\._reconstructor|pickle\.loads\s*\(|dill\.loads\s*\()").unwrap()
        });
        if let Some(m) = py_reduce.find(&decoded) {
            dets.push(mk(
                "python_reduce",
                0.90,
                &format!("Python deserialization primitive: {}", m.as_str()),
        ...
        static pickle_b64: std::sync::LazyLock<Regex> =
            std::sync::LazyLock::new(|| Regex::new(r"(?i)\bgAS[A-Za-z0-9+/=_-]{12,}\b").unwrap());
        ...
        if let Some(m) = pickle_opcode_rce.find(&decoded) {
            dets.push(mk(
                "python_pickle_opcode_rce",
                0.94,
                "Python pickle RCE opcode pattern (GLOBAL/REDUCE)",
        ...
        static dotnet: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r"(?i)(?:TypeConfuseDelegate|ObjectDataProvider|System\.Diagnostics\.Process|System\.Windows\.Data)").unwrap()
        });
        if let Some(m) = dotnet.find(&decoded) {
            dets.push(mk(
                "dotnet_deser",
                0.88,
                &format!(".NET deserialization gadget: {}", m.as_str()),
        ...
        static dotnet_gadget: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r"(?i)(?:ActivitySurrogateSelector|TextFormattingRunProperties)").unwrap()
        });
        if let Some(m) = dotnet_gadget.find(&decoded) {
            dets.push(mk(
                "dotnet_gadget_chain",
                0.92,
                &format!(".NET gadget class: {}", m.as_str()),
        ...
        static dotnet_binaryformatter: std::sync::LazyLock<Regex> =
            std::sync::LazyLock::new(|| {
                Regex::new(r"\bAAEAAAD/////[A-Za-z0-9+/=]{12,}\b").unwrap()
            });
        if let Some(m) = dotnet_binaryformatter.find(&decoded) {
            dets.push(mk(
                "dotnet_binaryformatter_payload",
                0.91,
                "Possible .NET BinaryFormatter payload blob",
        ...
        static yaml: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r"!!(?:python/object|ruby/object|java\.net|javax\.)").unwrap()
        });
        if let Some(m) = yaml.find(&decoded) {
            dets.push(mk(
                "yaml_deser",
                0.90,
                &format!("YAML deserialization payload: {}", m.as_str()),
        ...
        static yaml_lang_tags: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(
                r#"(?i)!!(?:python/object(?:/new|/apply)?|ruby/object|java(?:/[a-z0-9_.:$-]+)?)"#,
            )
            .unwrap()
        });
        ...
        static ruby_gadget: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r"(?i)(?:Gem::Installer|Gem::SpecFetcher|ERB)").unwrap()
        });
        ...
        static ruby_marshal: std::sync::LazyLock<Regex> =
            std::sync::LazyLock::new(|| Regex::new(r#"(?i)\bBAh(?:[A-Za-z0-9+/=]{10,})"#).unwrap());
        if let Some(m) = ruby_marshal.find(&decoded) {
            dets.push(mk(
                "ruby_marshal_payload",
                0.89,
                "Ruby Marshal payload marker (BAh...)",
        ...
        if has_ruby_marshal_magic || decoded.contains("\\x04\\x08") {
            let marker = if decoded.contains("\\x04\\x08") {
                "\\x04\\x08"
            } else {
                "\u{0004}\u{0008}"
            };
            ...
            dets.push(mk(
                ...
                0.93,
                "Ruby Marshal magic bytes detected (0x04 0x08)",
        ...
        if has_hessian_signature {
            dets.push(mk(
                "java_hessian_serial",
                0.89,
                "Hessian serialization signature detected",
                ...
                "Hessian",
                decoded.find("Hessian").unwrap_or(0),
            ));
        }

        // Node.js unserialize abuse marker from node-serialize style payloads
        static node_unserialize: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
            Regex::new(r"(?i)_\$\$ND_FUNC\$\$_|(?:node-serialize|serialize-javascript)").unwrap()
        });
    ...
    fn map_class(&self, detection_type: &str) -> Option<InvariantClass> {
        match detection_type {
            "java_serial"
            | "java_gadget_chain"
            | "java_ysoserial_gadget"
            | "java_polymorphic_type_abuse"
            | "ruby_yaml_gadget"
            | "ruby_marshal_payload"
            | "ruby_marshal_magic_bytes"
            | "java_kryo_serial"
...
mod tests {
    ...
    fn detects_php_pop_chain_namespace() {
        let eval = DeserEvaluator;
        let dets =
            eval.detect(r#"O:8:"Exploit":1:{s:4:"x";s:24:"Monolog\\Handler\\SyslogUdpHandler";}"#);
    ...
    fn detects_dotnet_and_ruby_gadgets() {
        ...
        assert!(
            dotnet
                .iter()
                .any(|d| d.detection_type == "dotnet_gadget_chain")
        );
    ...
    fn detects_java_ysoserial_and_polymorphic_abuse() {
        ...
        let poly = eval
            .detect(r#"{"@type":"com.sun.org.apache.xalan.internal.xsltc.trax.TemplatesImpl"}"#);
        assert!(
            ysoserial
                .iter()
                .any(|d| d.detection_type == "java_ysoserial_gadget")
        );
        assert!(
            poly.iter()
                .any(|d| d.detection_type == "java_polymorphic_type_abuse")
        );
    }
    ...
    fn detects_php_magic_and_phar() {
        ...
        assert!(
            magic
                .iter()
                .any(|d| d.detection_type == "php_magic_method_chain")
        );
    ...
    fn detects_pickle_base64_dotnet_blob_and_node_marker() {
        ...
        assert!(
            pickle
                .iter()
                .any(|d| d.detection_type == "python_pickle_base64")
        );
        assert!(
            dotnet
                .iter()
                .any(|d| d.detection_type == "dotnet_binaryformatter_payload")
        );
        assert!(
            node.iter()
                .any(|d| d.detection_type == "node_unserialize_gadget")
        );
    }
    ...
    fn detects_python_pickle_opcode_rce_patterns() {
        ...
        assert!(
            dets.iter()
                .any(|d| d.detection_type == "python_pickle_opcode_rce")
        );
    }
    ...
    fn detects_php_o_pattern_known_gadget_classes() {
        let eval = DeserEvaluator;
        let dets =
            eval.detect(r#"O:31:"Monolog\Handler\SyslogUdpHandler":1:{s:7:"socket";s:2:"fd";}"#);
        assert!(
            dets.iter()
                .any(|d| d.detection_type == "php_gadget_object_chain")
        );
    }
    ...
    fn detects_ruby_marshal_magic_bytes_marker() {
        ...
        assert!(
            dets.iter()
                .any(|d| d.detection_type == "ruby_marshal_magic_bytes")
        );
    }
    ...
    fn detects_dotnet_viewstate_binaryformatter_combo() {
        ...
        assert!(
            dets.iter()
                .any(|d| d.detection_type == "dotnet_viewstate_binaryformatter")
        );
    }
    ...
    fn detects_yaml_python_ruby_java_object_tags() {
        ...
        assert!(
            py.iter()
                .any(|d| d.detection_type == "yaml_language_tag_deser")
        );
        assert!(
            rb.iter()
                .any(|d| d.detection_type == "yaml_language_tag_deser")
