...
static TAG_INJECTION: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)<\s*(?:script|iframe|object|embed|applet|form|meta|link|style|base|svg|math|video|audio|source|details|marquee|isindex|frameset|frame|body|img|input|button|textarea|select|keygen)\b[^>]*>").unwrap()
});
static ATTRIBUTE_ESCAPE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"(?i)['"][\s/]*(?:>|on\w+\s*=|style\s*=|xmlns\s*=|src\s*=|href\s*=|action\s*=|formaction\s*=)"#).unwrap()
});
static EVENT_HANDLER: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)\bon(?:error|load|click|mouseover|mouseout|mousedown|mouseup|focus|blur|change|submit|reset|select|abort|unload|resize|scroll|keydown|keypress|keyup|dblclick|drag|drop|input|invalid|toggle|animationend|copy|cut|paste|search|wheel|contextmenu|auxclick)\s*=\s*[^\s>]").unwrap()
});
static PROTOCOL_HANDLER: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?i)(?:javascript|vbscript|livescript)\s*:|data\s*:\s*(?:text/html|application/xhtml)",
    )
    .unwrap()
});
static TEMPLATE_EXPR_1: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)\{\{.*(?:constructor|__proto__|prototype|\$on|\$emit|\$eval|alert|prompt|confirm|document|window|globalThis|Function).*\}\}").unwrap()
});
static TEMPLATE_EXPR_2: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)\$\{.*(?:alert|document|window|constructor|eval|Function)\s*\(.*\}\s*")
        .unwrap()
});
