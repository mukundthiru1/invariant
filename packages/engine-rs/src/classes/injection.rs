...
static PROTO_BASE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"__proto__|constructor\s*\[\s*['"]?prototype['"]?\s*\]|constructor\.prototype|Object\.assign.*__proto__"#).unwrap()
});
static LOG4SHELL_A: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"\$\{(?:jndi|lower|upper|env|sys|java|date|main|bundle|ctx|spring|kubernetes|docker|log4j)[\s:]").unwrap()
});
static LOG4SHELL_B: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\$\{.*?\$\{").unwrap());
static SSTI_JINJA_A: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"\{\{.*(?:__class__|__mro__|__subclasses__|__builtins__|__globals__|config|lipsum|cycler|joiner|namespace|request\.|self\.).*\}\}").unwrap()
});
static SSTI_JINJA_B: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\{%.*(?:import|include|extends|block|macro|call).*%\}").unwrap());
static SSTI_JINJA_C: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\{\{.*(?:\d+\s*[+\-*/]\s*\d+).*(\}\}).*\{\{.*\|.*\}\}").unwrap());
static SSTI_EL_A: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"\$\{.*(?:Runtime|ProcessBuilder|exec|getClass|forName|getMethod|invoke).*\}")
        .unwrap()
});
static SSTI_EL_B: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"#\{.*(?:T\(|new |java\.).*\}").unwrap());
static SSTI_EL_C: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"%\{.*(?:#cmd|#context|#attr|@java).*\}").unwrap());
static NOSQL_OP_A: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"\$(?:gt|gte|lt|lte|ne|eq|in|nin|regex|exists|type|where|or|and|not|nor|elemMatch)\b",
    )
    .unwrap()
});
static NOSQL_OP_B: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"\{"?\$(?:gt|ne|regex|where)"?\s*:"#).unwrap());
static NOSQL_JS_A: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"["']?\$where["']?\s*:\s*["']?(?:function|this\.|sleep|db\.|emit|tojson)"#)
        .unwrap()
});
...
static XXE_A: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<!(?:DOCTYPE|ENTITY)\s+\S+\s+(?:SYSTEM|PUBLIC)\s+["'][^"']*["']"#).unwrap()
});
static XXE_B: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<!ENTITY\s+\S+\s+["'](?:file:|http:|ftp:|php:|expect:|data:)"#).unwrap()
});
...
fn proto_pollution_gadget(input: &str) -> bool {
    let d = decode(input);
    if !Regex::new(
        r#"__proto__|constructor\s*\[\s*['"]?prototype['"]?\s*\]|constructor\.prototype"#,
    )
    .unwrap()
    .is_match(&d)
    {
    ...
    for cap in Regex::new(r#"__proto__\[['"]?([a-zA-Z_$][a-zA-Z0-9_$]*)['"]?\]"#)
        .unwrap()
        .captures_iter(&d)
    {
    ...
    for cap in Regex::new(r"constructor\.prototype\.([a-zA-Z_$][a-zA-Z0-9_$]*)")
        .unwrap()
        .captures_iter(&d)
    {
        targets.push(cap[1].to_string());
    ...
    for cap in Regex::new(r#""__proto__"\s*:\s*\{([^}]*)\}"#)
        .unwrap()
        .captures_iter(&d)
    {
        if let Some(inner) = cap.get(1) {
            for key in Regex::new(r#""([a-zA-Z_$][a-zA-Z0-9_$]*)"\s*:"#)
                .unwrap()
    ...
    targets
        .into_iter()
        .any(|t| gadgets.iter().any(|g| g.eq_ignore_ascii_case(&t)))
}
...
fn crlf_header_injection(input: &str) -> bool {
    ...
    Regex::new(r"%0[dD]%0[aA]").unwrap().is_match(input)
        || (Regex::new(r"\r\n").unwrap().is_match(&d)
            && Regex::new(r"(?:Set-Cookie|Location|Content-Type|HTTP/)")
                .unwrap()
                .is_match(&d))
...
fn crlf_log_injection(input: &str) -> bool {
    let d = decode(input);
    let has_newline = Regex::new(r"%0[aAdD]").unwrap().is_match(input)
        || Regex::new(r"[\r\n]").unwrap().is_match(&d);
    has_newline
        && (Regex::new(r"\[(?:INFO|WARN|ERROR|DEBUG|ALERT|CRITICAL|NOTICE)\]")
            .unwrap()
            .is_match(&d)
            || Regex::new(r"\d{4}-\d{2}-\d{2}[\sT]\d{2}:\d{2}")
                .unwrap()
                .is_match(&d)
            || Regex::new(r"\[\d{4}-\d{2}-\d{2}\]").unwrap().is_match(&d)
            || Regex::new(r"(?:ADMIN|ACCESS|GRANTED|DENIED|LOGIN|LOGOUT|BYPASS|ELEVATED)")
                .unwrap()
                .is_match(&d))
}
...
fn graphql_introspection(input: &str) -> bool {
    ...
    Regex::new(r"__schema\s*\{").unwrap().is_match(&d)
        ...
        || Regex::new(r"\{\s*__schema\s*\{.*queryType")
            .unwrap()
            .is_match(&d)
}
...
fn graphql_batch_abuse(input: &str) -> bool {
    let d = decode(input);
    let alias_count = Regex::new(r"\w+\s*:\s*\w+\s*\(")
        .unwrap()
        .find_iter(&d)
        .count();
    alias_count >= 5
        || Regex::new(r"(?s)^\s*\[.*\{.*query.*\}.*\{.*query.*\}")
            .unwrap()
            .is_match(&d)
}
...
fn open_redirect_bypass(input: &str) -> bool {
    ...
    (Regex::new(r"//[^/]+\.[^/]+").unwrap().is_match(&d)
        && Regex::new(r"(?:redirect|url|next|return|goto|dest|target|rurl|forward)\s*[=:]")
            .unwrap()
            .is_match(&d))
        || Regex::new(r"\\\\[^\\]+\\").unwrap().is_match(&d)
        || Regex::new(r"(?:redirect|url|next|goto)=(?://|https?:|%2[fF]%2[fF])")
            .unwrap()
            .is_match(input)
}
...
fn http_smuggle_cl_te(input: &str) -> bool {
    ...
    if Regex::new(r"Transfer-Encoding\s*:")
        .unwrap()
        .find_iter(&d)
        .count()
        >= 2
    ...
    Regex::new(r"Transfer[\s-]*Encoding\s*:\s*chunked")
        .unwrap()
        .is_match(&d)
...
fn http_smuggle_h2(input: &str) -> bool {
    let d = decode(input);
    if Regex::new(r":method\s|:path\s|:authority\s|:scheme\s")
        .unwrap()
        .is_match(&d)
        && Regex::new(r"Transfer-Encoding|Content-Length")
            .unwrap()
            .is_match(&d)
    {
        return true;
    }
    if Regex::new(r"(?:GET|POST|PUT|DELETE|PATCH|OPTIONS|HEAD)\s+/[^\s]*\s+HTTP/\d")
        .unwrap()
        .find_iter(&d)
        .count()
        >= 2
    ...
    if Regex::new(r":authority\s").unwrap().is_match(&d)
        && Regex::new(r"\bHost\s*:").unwrap().is_match(&d)
    {
        return true;
    }
...
fn http_smuggle_chunk_ext(input: &str) -> bool {
    ...
    Regex::new(r"\b[1-9a-fA-F][0-9a-fA-F]*\s*;[^\r\n]+\r?\n")
        .unwrap()
        .is_match(&d)
        && Regex::new(r"(?:GET|POST|PUT|DELETE|PATCH)\s+/")
            .unwrap()
            .is_match(&d)
}
...
fn http_smuggle_zero_cl(input: &str) -> bool {
    ...
    if let Some(m) = Regex::new(r"Content-Length:\s*0\s*\r?\n").unwrap().find(&d) {
        if let Some(i) = d[m.start()..].find("\r\n\r\n") {
            ...
            if !body.is_empty()
                && Regex::new(r"(?:GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)\s+/")
                    .unwrap()
                    .is_match(body)
            {
                return true;
...
fn http_smuggle_expect(input: &str) -> bool {
    ...
    let request_lines =
        Regex::new(r"(?:GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)\s+/[^\s]*\s+HTTP/\d")
            .unwrap()
            .find_iter(&d)
            .count();
    if request_lines >= 2 {
    ...
    Regex::new(r"Content-Length").unwrap().is_match(&d)
        && Regex::new(r"Transfer-Encoding").unwrap().is_match(&d)
}
...
fn dependency_confusion(input: &str) -> bool {
    ...
    if !has_lodash_typosquat {
        for cap in Regex::new(r#"\brequire\(\s*['"]([^'"]+)['"]\s*\)"#)
            .unwrap()
            .captures_iter(&d)
        {
            let pkg = cap.get(1).map(|m| m.as_str()).unwrap_or("");
...
fn postinstall_injection(input: &str) -> bool {
    let d = decode(input);
    let re =
        Regex::new(r#""(?:preinstall|postinstall|install)"\s*:\s*"((?:[^"\\]|\\.)*)""#).unwrap();
    for cap in re.captures_iter(&d) {
        ...
        if Regex::new(r"\bnode\s+-e\b").unwrap().is_match(&raw)
            && Regex::new(r"\beval\b").unwrap().is_match(&raw)
        {
            return true;
        }
        if Regex::new(r"\b(eval|sh\s+-c|bash\s+-c)\b")
            .unwrap()
            .is_match(&raw)
            && Regex::new(r"(?:\$\(|`[^`]+`|\\x[0-9a-f]{2}|base64)")
                .unwrap()
                .is_match(&raw)
        {
            return true;
...
fn ws_injection(input: &str) -> bool {
    ...
    let looks_like_ws = Regex::new(r"\{[\s\S]*\}").unwrap().is_match(&d)
        || Regex::new(r"(?i)(?:websocket|ws[_-]?(?:message|frame))")
            .unwrap()
            .is_match(&d);
    if !looks_like_ws {
        return false;
...
fn llm_data_exfiltration(input: &str) -> bool {
    let d = decode(input).to_ascii_lowercase();
    let exfil =
        Regex::new(r"\b(confidential|internal|proprietary|restricted|private|secret)\b").unwrap();
...
fn llm_jailbreak(input: &str) -> bool {
    let d = decode(input);
    if Regex::new(r"(?i)\bDAN\b[\s\S]{0,80}\b(?:mode|now|prompt|jailbreak|anything now)\b")
        .unwrap()
        .is_match(&d)
    {
        return true;
    }
    if Regex::new(r"(?i)\bSTAN\b[\s\S]{0,80}\b(?:mode|now|prompt|jailbreak)\b")
        .unwrap()
        .is_match(&d)
    {
        return true;
    }
    if Regex::new(r"(?i)\bDUDE\b[\s\S]{0,80}\b(?:mode|now|prompt|jailbreak)\b")
        .unwrap()
        .is_match(&d)
    {
        return true;
    ...
    if Regex::new(
        r"(?i)\[jailbreak\][\s\S]{0,80}\b(?:ignore|disregard|policy|content|instruction|prompt)\b",
    )
    .unwrap()
    .is_match(&d)
    {
    ...
    if Regex::new(r"(?i)\b(?:enable|activate)\s+developer\s+mode\b")
        .unwrap()
        .is_match(&d)
    ...
    if Regex::new(r"(?i)\bdeveloper\s+mode\s+enabled\b")
        .unwrap()
        .is_match(&d)
    {
        return true;
...
fn cache_poisoning(input: &str) -> bool {
    ...
    Regex::new(r"<script|evil\.|attacker\.|malicious\.|nothttps?|/admin|javascript:")
        .unwrap()
        .is_match(&d)
}
...
fn cache_deception(input: &str) -> bool {
    let d = decode(input);
    let dynamic = Regex::new(
        r"(?:/api/|/account/|/user/|/profile/|/settings/|/admin/|/my-?account/|/dashboard/)",
    )
    .unwrap();
    ...
    let ext = Regex::new(
        r"\.(?:css|js|png|jpg|jpeg|gif|svg|ico|woff2?|ttf|eot|map|json)(?:\?|#|%23|\s|$)",
    )
    .unwrap();
    ext.is_match(&d)
}
...
fn bola_idor(input: &str) -> bool {
    ...
    if has_numeric {
        let auth_mismatch = Regex::new(
            r"(?:token[_\s]*for[_\s]*user|bearer\s+<|as\s+user\s+\d|impersonat|other[_\s]*user)",
        )
        .unwrap()
        .is_match(&d);
        let sequential_probe = Regex::new(r"(?:sequential|probe|enumerate|brute|scan|fuzz)")
            .unwrap()
            .is_match(&d);
    ...
    if Regex::new(r"/api/.*\.\./").unwrap().is_match(&d)
        && Regex::new(r"(?:admin|config|internal|private|secret)")
            .unwrap()
            .is_match(&d)
    {
        return true;
...
fn api_mass_enum(input: &str) -> bool {
    let d = decode(input);
    let calls: Vec<i64> = Regex::new(r"/api/\w+/(\d+)")
        .unwrap()
        .captures_iter(&d)
    ...
    let range_start = Regex::new(r"(?:id|_id)\s*\[?\s*(?:gte|gt)\s*\]?\s*[=:]\s*(\d+)")
        .unwrap()
        .captures(&d);
    let range_end = Regex::new(r"(?:id|_id)\s*\[?\s*(?:lte|lt)\s*\]?\s*[=:]\s*(\d+)")
        .unwrap()
        .captures(&d);
    ...
    Regex::new(r"\bfilter\s*=\s*(?:id|_id)\s*[>]=?\s*0\b")
        .unwrap()
        .is_match(&d)
}
...
pub const INJECTION_CLASSES: &[ClassDefinition] = &[
    ...
    ClassDefinition {
        ...
        formal_property: Some(
            "∃ key_path ∈ parse(input, PROPERTY_ACCESS_GRAMMAR) : key_path REACHES Object.prototype ∧ target(key_path) ∈ GADGET_DATABASE",
        ),
        composable_with: &[
    ...
    ClassDefinition {
        ...
        cwe: Some("CWE-444"),
        formal_property: Some(
            "∃ headers ∈ parse(request, HTTP_HEADER_GRAMMAR) : ('Content-Length' ∈ keys(headers) ∧ 'Transfer-Encoding' ∈ keys(headers))",
        ),
        composable_with: &[
    ...
    ClassDefinition {
        ...
        cwe: Some("CWE-444"),
        formal_property: Some(
            "∃ pseudoHeader ∈ {':method', ':path', ':authority', ':scheme'} : CRLF ∈ value(pseudoHeader)",
        ),
        composable_with: &[
    ...
    ClassDefinition {
        ...
        cwe: Some("CWE-444"),
        formal_property: Some(
            "∃ chunk ∈ parse(body, CHUNKED_ENCODING_GRAMMAR) : chunk.extensions.length > 0",
        ),
        composable_with: &[
            InvariantClass::HttpSmuggleClTe,
            InvariantClass::HttpSmuggleH2,
    ...
    ClassDefinition {
        ...
        cwe: Some("CWE-444"),
        formal_property: Some(
            "∃ request ∈ parse(input, HTTP_GRAMMAR) : header(request, 'Content-Length') = '0' ∧ body(request).length > 0",
        ),
        composable_with: &[
            InvariantClass::HttpSmuggleClTe,
            InvariantClass::HttpSmuggleChunkExt,
    ...
    ClassDefinition {
        ...
        cwe: Some("CWE-444"),
        formal_property: Some(
            "∃ request ∈ parse(input, HTTP_GRAMMAR) : header(request, 'Expect') = '100-continue'",
        ),
        composable_with: &[
            InvariantClass::HttpSmuggleClTe,
            InvariantClass::HttpSmuggleZeroCl,
