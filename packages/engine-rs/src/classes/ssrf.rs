...
static INTERNAL_REACH: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?:https?://)?(?:127\.0\.0\.1|localhost|0\.0\.0\.0|10\.\d+\.\d+\.\d+|172\.(?:1[6-9]|2\d|3[01])\.\d+\.\d+|192\.168\.\d+\.\d+|0x7f|2130706433|017700000001|\[::1?\]|0177\.0\.0\.01)").unwrap()
});
static CLOUD_METADATA: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"169\.254\.169\.254|metadata\.google\.internal|100\.100\.100\.200|fd00:ec2::254|metadata\.azure\.com").unwrap()
});
static PROTOCOL_SMUGGLE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?:file|gopher|dict|ldap|tftp|ftp|jar|netdoc|phar)://").unwrap());
