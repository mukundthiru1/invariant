...
static DOTDOT_DECODED: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?:\.{2,}[\\/]+){2,}").unwrap());
static DOTDOT_RAW: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?:\.\.%2[fF]|%2[eE]%2[eE]%2[fF]|\.\.%5[cC]){2,}").unwrap());
static NULL_TERM: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"%00|\\x00|\\0|\x00").unwrap());
static ENC_BYPASS: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"%252[eE]%252[eE]|%25252|%c0%ae|%c0%af|%e0%80%ae|\.%00\.").unwrap());
static SENSITIVE_PATH: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"/etc/(?:passwd|shadow|hosts)|/proc/self/(?:environ|cmdline|maps)|/windows/(?:system32|win\.ini)").unwrap());
static TRAVERSAL_HINT: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?:\.\.[\\/]|%2e|%252e)").unwrap());
...
fn path_normalization_bypass(input: &str) -> bool {
    let d = decode(input);
    Regex::new(r"/[^/]+\.{1,}/").unwrap().is_match(&d)
        || Regex::new(r"/(?:admin|config|internal|secret|private|\.env|\.git)/\.(?:/|$)").unwrap().is_match(&d)
        || Regex::new(r"/[^/]+\s+/").unwrap().is_match(&d)
...
pub const PATH_CLASSES: &[ClassDefinition] = &[
    ClassDefinition {
        ...
        known_payloads: &["../../../etc/passwd", "..\\..\\..\\windows\\win.ini", "....//....//....//etc/passwd", "..%2F..%2F..%2Fetc%2Fpasswd"],
        known_benign: &["/home/user/documents", "file.txt", "./local-file.js", "../sibling-dir", "https://example.com/path"],
        mitre: &["T1083"],
        cwe: Some("CWE-22"),
    ...
    ClassDefinition {
        ...
        known_payloads: &["../../../etc/passwd%00.jpg", "shell.php%00.gif", "/etc/passwd\\x00.html"],
    ...
    ClassDefinition {
        ...
        known_payloads: &["%252e%252e%252fetc%252fpasswd", "..%c0%af..%c0%afetc/passwd", "..%e0%80%ae/etc/passwd"],
        known_benign: &["hello%20world", "/api/users", "filename.txt", "%E2%9C%93 check mark"],
        mitre: &["T1083"],
        cwe: Some("CWE-22"),
        formal_property: None,
        composable_with: &[],
    ...
    ClassDefinition {
        ...
        known_payloads: &["/admin/./", "/admin../", "/Admin%20/", "/admin;/secret", "/admin;jsessionid=x/secret", "/api/v1/admin\\secret"],
        known_benign: &["/api/users", "/home/page", "/about", "/contact-us", "/images/logo.png"],
        mitre: &["T1083"],
        cwe: Some("CWE-22"),
        formal_property: None,
        composable_with: &[],
