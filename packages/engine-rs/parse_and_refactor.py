...
for v in variants:
    ...
    mitre_entry = mitre_map.get(v, {'technique': 'T1190', 'rationale': 'Generic mapping.'})
    ...
    # Generic tags for compliance based on basic rules matching `compliance.rs`
...
# Replace `InvariantClass::all_variants()` with ALL_CLASSES array since `all_variants` doesn't exist
all_classes_arr = "pub const ALL_CLASSES: &[InvariantClass] = &[\n" + ",\n".join("    InvariantClass::" + v for v in variants) + "\n];"
...
print("Generated src/class_registry.rs")

# 7. Refactor types.rs
# We will inject `pub fn attack_category` etc., and replace `category()`, `default_severity()`, `severity_weight()` match arms!
types_rs = re.sub(
...
print("Refactored src/types.rs")

# 8. Refactor compliance.rs
with open('src/compliance.rs', 'r') as f:
    comp_rs = f.read()

comp_rs = re.sub(r'const ALL_CLASSES.*?\];\n*', '', comp_rs, flags=re.DOTALL)
comp_rs = re.sub(r'fn owasp_top10_2021_for_class.*?\}', '', comp_rs, flags=re.DOTALL)
comp_rs = re.sub(r'fn owasp_api_top10_for_class.*?\}', '', comp_rs, flags=re.DOTALL)
comp_rs = re.sub(r'fn cwe_ids_for_class.*?\}', '', comp_rs, flags=re.DOTALL)
comp_rs = re.sub(r'fn framework_controls_for_class.*?Vec<ComplianceControl> \{.*?\n\}\n', '', comp_rs, flags=re.DOTALL)
...
# Insert `controls_code` where `pub fn compliance_mapping_for_class` is defined
comp_rs = comp_rs.replace('pub fn compliance_mapping_for_class(class: InvariantClass) -> ComplianceMapping {', controls_code + '\npub fn compliance_mapping_for_class(class: InvariantClass) -> ComplianceMapping {')

# fix ALL_CLASSES usage in tests
comp_rs = comp_rs.replace('&ALL_CLASSES', 'crate::class_registry::ALL_CLASSES')

with open('src/compliance.rs', 'w') as f:
    f.write(comp_rs)
print("Refactored src/compliance.rs")
